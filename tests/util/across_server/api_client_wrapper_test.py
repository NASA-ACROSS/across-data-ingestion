...
class TestGetClient:
    ...
    def test_should_return_an_existing_client_instance(
        ...
        assert second == mock_wrapper_cls.return_value
    ...
    def test_should_return_a_new_instance_when_it_dne(
        ...
        assert client == mock_wrapper_cls.return_value


class TestRefresh:
    def test_should_only_rotate_the_key_once_when_using_thread_lock(
        self, test_api_client: ApiClientWrapper, mock_key_manager: MagicMock
    ):
        ...
        assert mock_key_manager._rotation_count["count"] == 1
